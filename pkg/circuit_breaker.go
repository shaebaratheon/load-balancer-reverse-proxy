...
func (c *CircuitBreakerContext) RecordTransition(state string) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (e *CircuitBreakerEngine) Register(key, val string) bool {
	e.mu.Lock()
	defer e.mu.Unlock()
...
func (e *CircuitBreakerEngine) Process(ctx *CircuitBreakerContext) (string, error) {
	e.mu.Lock()
	defer e.mu.Unlock()
	e.processedCount++
...
func (e *CircuitBreakerEngine) Health() (int, int, bool) {
	e.mu.RLock()
	defer e.mu.RUnlock()
